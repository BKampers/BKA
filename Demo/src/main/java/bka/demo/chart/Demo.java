...
public class Demo extends javax.swing.JFrame {
    ...
    private void configureLineChart(Map<Object, Map<Number, Number>> graphs) {
        ...
        chartRenderer.setGraphs(graphs);
        PolygonDotRenderer markerRenderer = new PolygonDotRenderer(createStar(5, 3, 9), DefaultDrawStyle.createSolid(Color.BLACK));
    ...
    private class DataMiner extends TimerTask {
        ...
        public void run() {
            if (styleComboBox.getSelectedItem() == GraphStyle.DOT) {
                Number lastTimestamp = new LinkedList<>(months.keySet()).getLast();
                Calendar calendar = Calendar.getInstance();
                calendar.setTimeInMillis(lastTimestamp.longValue());
                calendar.add(Calendar.MONTH, 1);
                months.put(calendar.getTimeInMillis(), calendar.get(Calendar.MONTH));
                Map<Object, Map<Number, Number>> graphs = new LinkedHashMap<>();
                graphs.put(MONTHS, months);
    ...
    private static PolygonDotRenderer createPolygonDotRenderer() {
        return new PolygonDotRenderer(createStar(4, 5, 15), createPointDrawStyle());
    }

    private static Polygon createStar(int count, double innerRadius, double outerRadius) {
        return PolygonFactory.toPolygon(PolygonFactory.createStar(count, innerRadius, outerRadius));
    }
